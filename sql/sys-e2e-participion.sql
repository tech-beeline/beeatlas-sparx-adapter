...
cte_sys AS (
	...
	FROM cte_sys_catalog cat
		JOIN t_object sys ON sys.package_id=cat.package_id AND sys.alias = 'RICH' AND sys.object_type='Component'
),
cte_realization AS ( 
...
cte_sys_obj AS (
	SELECT  sys.name, sys.code AS sys_code, sys.ea_guid, sys.code, sys.sys_id, sys.sys_id as object_id
	FROM cte_sys sys
	UNION -- Provided interfaces
	SELECT it.name, sys.code, it.ea_guid, it.alias, sys.sys_id, it.object_id
	FROM cte_sys sys
	...
	SELECT c.name, sys.code, c.ea_guid, c.alias, sys.sys_id, c.object_id
	FROM cte_sys sys
...
cte_dia_ref AS (
	SELECT od.diagram_id, d.name , d.diagram_id AS child_id
	FROM t_xref x
...
cte_bi_dia AS (
	SELECT bi.process, bi.process_uid, bi.bi_name, bi.bi_id, bi.bi_id as diagram_id, bi.bi_name as diagram, bi.bi_id as child_id
	FROM cte_bi bi
	UNION DISTINCT
	SELECT bi.process, bi.process_uid, bi.bi_name,bi.bi_id,ref.diagram_id, ref.name, ref.child_id
	FROM cte_bi_dia bi
		JOIN cte_dia_ref ref ON ref.diagram_id=bi.child_id
), cte_sys_msg AS (
	SELECT msg.name, mtd.name as method, it.name as interface, op.value as operation_guid,
	o.*,
	d.*
	FROM cte_bi_dia d
