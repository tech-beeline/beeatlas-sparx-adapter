...
// [ ] Исправить обновление при выборе другой системы
...
export default function SystemAssessmentsAccordion({ system }) {
    ...
    return (
        <Accordion>
            ...
            {error ? <AccordionDetails color="red"><Alarm />{error.message}</AccordionDetails> : null}
            {data ?
                <AccordionDetails>
                    <Table size="small">
                        <TableHead>
                        ...
                        <TableBody >
                            {data.map(ar =>
                                <TableRow hover key={ar => ar.fitness_function_code} sx={{ backgroundColor: ar.status?"#FFEBEE":"#E8F5E9" }}>
                                    ...
                                    <TableCell>{ar.status ? "Есть ошибки" : "Успешно"}</TableCell>
