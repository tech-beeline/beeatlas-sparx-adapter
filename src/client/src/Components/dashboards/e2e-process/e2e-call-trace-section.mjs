import { AccountTree, Check, ExpandMore, KeyboardArrowDown, KeyboardArrowUp, Warning, WarningAmber, Menu as MenuIcon, Handyman } from '@mui/icons-material';
import { Accordion, AccordionDetails, AccordionSummary, Box, Button, Collapse, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, IconButton, List, ListItem, Menu, MenuItem, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography } from '@mui/material';
...
import webeaLogo from '../../images/ea-icon.ico'
    ...
    return (
        <Box>
            <Table>
                <TableBody>
                    <TableRow>
                        <TableCell>Сообщение</TableCell><TableCell>{messageDetails.message?.name}</TableCell>
                    </TableRow>
                </TableBody>
            </Table>
            ...
            <Table component={Paper}>
                <TableHead>
                    <TableRow>
                        <TableCell>Метод</TableCell><TableCell></TableCell>
                    </TableRow>
                </TableHead>
                <TableBody>
                    {messageDetails.server_methods.map(m => (
                        <TableRow>
                            <TableCell><Typography color="red">{m.name}</Typography></TableCell>
                            <TableCell><Button>Установить</Button></TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
...
function ErrorDetails({ call, error, setOpen }) {
    ...
    async function loadMessageDetails() {
        console.log('!')
        const response = await fetch(`/api/v3/e2e/messages/${encodeURIComponent(call.ea_guid)}`)
        if (response.status !== 200) {
            setMessageDetails({ error: `HTTP STATUS: ${response.status} ( ${response.statusText})`, errorBody: await response.text() })
            return;
        }
    ...
    return (
        <Dialog
            ...
            <DialogContent>
                <DialogContentText>Общее описание проблемы: {ERROR_INFO[error]?.summary ?? error}</DialogContentText>
                {messageDetails ? messageDetails.error ? `Ошибка при загрузке данных: ${messageDetails.error}` : <MessageDetails messageDetails={messageDetails} /> : "Данные загружаются"}
            </DialogContent>
            <DialogActions>
...
function Errors({ call }) {
    ...
    return (
        errors ? <>{errors.map((e, i) => <TreeItem sx={{ color: "red" }} label={label(e)} nodeId={`${call.ea_guid}-err-${i}`} key={`${call.ea_guid}-err-${i}`}></TreeItem>)}</> : null
    )
}
