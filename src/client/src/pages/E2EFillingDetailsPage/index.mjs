...
export function E2EFillingDetailsPage(props) {
    const [e2eStatus, setE2eStatus] = useState();

    const { code } = useParams();
    const loadNoteStatus = async () => {
        const response = await fetch(
            `/api/process-filling/${encodeURIComponent(code)}/details`
        );
            ...
            <table>
                <thead>
                    <tr>
                        <th>№</th>
                        <th>Процесс</th>
                        <th>Выключены заметки</th>
                        <th>Всего компонент</th>
                        <th>Компоненты не из справочника</th>
                        <th>Всего взаимодействий</th>
                        <th>Методы не из спецификации</th>
                        <th>Без IA</th>
                    </tr>
                </thead>
                {e2eStatus.map((row, i) => (
                    <tr className="hl-row">
                        <td>{i + 1}</td>
                        <td>
                            <NavLink to={row.uid}>{row.sequence}</NavLink>
                        </td>
                        <td>
                            {row.diagrams_notes_off ? (
                                <font color="red">
                                    <b>Выключены</b>
                                </font>
                            ) : (
                                <font color="green">Включены</font>
                            )}
                        </td>
                        <td>{row.total_components}</td>
                        <td>
                            {row.components_not_from_catalog > 0 ? (
                                <b style={{ color: "red" }}>
                                    {row.components_not_from_catalog}
                                </b>
                            ) : (
                                0
                            )}
