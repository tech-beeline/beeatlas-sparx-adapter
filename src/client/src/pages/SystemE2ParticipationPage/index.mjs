...
export function SystemE2EParticipion({ systemCode }) {
    ...
    for (const row of participationList ?? []) {
        if (!operationMap[row.message.operation.name])
            operationMap[row.message.operation.name] = row.message.operation;

        if (
            operationFilter.length &&
            !operationFilter.some((m) => row.message.operation.name === m)
        ) {
            continue;
        }
        const process =
            processMap[row.process.uid] ??
            (processMap[row.process.uid] = { children: [], ...row.process });
        const operation_uid = row.message.operation?.uid;
        if (operation_uid) {
            let operation = process.children.find(
                (r) => r.uid === operation_uid
            );
            if (!operation) {
    ...
    const processRows = (process) => {
        return process.children.map((r, i) =>
            i ? (
                ...
                <TableRow key={i}>
                    <TableCell rowSpan={process.children.length}>
                        {process.name}
                    </TableCell>
                    <TableCell>{r.name}</TableCell>
                    <TableCell>{r.interface.name}</TableCell>
    ...
    const loadingAccodrion = loading ? (
        <AccordionDetails></AccordionDetails>
    ) : null;
    const errorDetails = error ? (
        <AccordionDetails>Ошибка: {error}</AccordionDetails>
    ) : null;
    const participationDetails = participationList ? (
        <AccordionDetails>
            <SelectOperation
                value={operationFilter}
                setValue={setOperationFilter}
...
export function SystemE2EParticipionPage() {
    ...
    const breadcrumbs = (
        <Breadcrumbs aria-label="breadcrumb">
            ...
            <SystemSelect
                system={{ label: system?.name, code: system?.code }}
