...
export function E2EScenarioPage() {
    ...
    const loadScenario = async () => {
        const response = await fetch(
            `/api/v1/e2e-process-messages/${encodeURIComponent(uid)}`
        );
    ...
    function showHideApplication() {
        ...
        if (app_container) {
            app_container.style.display =
                app_container.style.display === "none" ? "" : "none";
    ...
    function showHideIA(e) {
        ...
        div_caption.parentElement
            .querySelectorAll(".ia-content")
            .forEach((d) => {
                d.style.display = d.style.display == "none" ? "" : "none";
                div_caption.innerText =
                    d.style.display === "none"
                        ? IA_CAPTION_TEXT.none
                        : IA_CAPTION_TEXT.show;
            });
    }

    function buildMessageTree(messages) {
        messages = messages
            ? messages.filter((m) => m.type !== "internalCall")
            : [];
        ...
        return (
            <>
                <ul>
                    {messages.map((m) => {
                        const has_child = m.messages?.filter(
                            (m) => m.type !== "internalCall"
                        ).length;
                        ...
                        return (
                            <li>
                                <MessageCard
                                    message={m}
                                    applications={
                                        e2eScenario?.scenario?.applications
                                    }
                                ></MessageCard>
                                {m.duration ? (
                                    <div>Длительность : {m.duration}</div>
                                ) : (
                                    ""
                                )}
                            </li>
            ...
            {e2eScenario?.scenario ? (
                <div>
                    ...
                    <div>
                        ...
                        <div>
                            {(
                                e2eScenario.scenario.businessInteractions ?? []
                            ).map((bi) => (
                                <div>
                                    <h4>{bi.name}</h4>
                                    <div>{buildMessageTree(bi.scenario)}</div>
                                </div>
                            ))}
                        </div>
                    </div>
