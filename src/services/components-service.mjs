...
class ComponentsService {
	...
	async getSystem(code) {
		 ...
		 */
		let system = null;
		for (const row of await Repository.queryRows({ text: SYSTEM_QUERY.QUERY_SYSTEM_REALIZATION, values: [code] })) {
			system = system ?? new System({ name: row.system, code: row.system_cmdb, version: row.sys_version });
			/**
			 * @type {Container}
			 */
			...
			let api = container.interfaceByCode(row.i_code) ?? container.addInterface({ name: row.interface, code: row.interface_code, version: row.interface_version });
		}
		return system;
	}
	/**
