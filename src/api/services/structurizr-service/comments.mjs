 ...
 * @returns 
 */
export const apiWithWrongTC = (s, c, api) => errorComment(`У интерфейса ${api.name} указан не существующий код ТС`,
    `Контейнер: ${c.name}
Так как для этого интерфейса указан не правильный код ТС, то информация не будет загружена о всех интерфейсах
...
export const scriptLineComment = () => errorComment(`Скрипт обработки SLA вызывается слишком рано`,
    `Для того, что бы скрипт запуска корректно обработал файл с sla, он должен вызываться после описания всех интерфейсов.

Рекомендуется поместить его после описания модели:

    model {
      // Описание контейнеров и интерфейсов
    }

    <mark>!script process-sla.groovy</mark>

    views {
    }
`, `Скрипты обработки SLA`);


export const scriptNotFoundComment = () => errorComment(`Скрипт обработки SLA не найден`,
    `
Для выгрузки SLA в витрину ФДМ требуется добавить в dsl вызов скрипты обработки SLA
При этом, что бы скрипт запуска корректно обработал файл с sla, он должен вызываться после описания всех интерфейсов.
