...
export async function removeInterface(container, api) {
    ...
    await interfaceRepository.delete(container.container_id, api);
}
...
export async function removeContainer(app, container) {
    ...
    for (const api_code in container.interfaces ?? {}) {
        await removeInterface(container, container.interfaces[api_code]);
    }

    return containerRepository.delete(app.object_id, container);
}