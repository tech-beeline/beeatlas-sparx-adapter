...
export default class OSLC {
    ...
    static #getUserId(xml) {
        let js_result = XMLJS.xml2js(xml, { compact: true });
    ...
    static async connect(endpoint, uid, password) {
        let auth_result = await request(`${endpoint}${AUTH_PATH}`, { method: "POST" }, `uid=${uid};pwd=${password};`)
        return new OSLC(endpoint, OSLC.#getUserId(auth_result));
    }
    async resourceShape() {
        let result = await request(`${this.#endpoint}${RESOURCE_SHAPE_PATH}?useridentifier=${this.#userId}`, { method: "GET" });
        return result;
    }
     ...
     * 
     * @param {any} resource 
     * @param {{ packageGUID:string?, elementGUID :string?} |null} parentResource 
     */
    async createResource(resource, parentResource) {
        ...
        let response = await request(`${this.#endpoint}${RESOURCE_FACTORY_PATH}`, { method: "POST", headers: { "Content-Type": "text/xml" } }, request_body.toString());
     ...
     */
    async createElement(element, packageGUID) {
        return this.createResource(element, { packageGUID: packageGUID });
    }
};
